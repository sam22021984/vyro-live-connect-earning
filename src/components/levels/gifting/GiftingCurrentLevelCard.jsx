...
export default function GiftingCurrentLevelCard() {
  ...
  const nextTier = giftingTiers[currentTier.tier] || giftingTiers[giftingTiers.length - 1];
  ...
  return (
    <div className="rounded-3xl overflow-hidden p-5" style={{ background: "linear-gradient(135deg, #FFFFFF 0%, #F5F7FA 100%)", border: "1px solid rgba(255,255,255,0.9)", boxShadow: `0 12px 36px ${c.glow}` }}>
      <div className="flex items-center justify-between mb-4">
        <div>
          <p className="text-[10px] text-gray-400 font-medium uppercase tracking-wider">Gifting Level</p>
      ...
      <div className="mb-3">
        <div className="flex items-center justify-between mb-1.5">
          <div className="flex items-center gap-1"><Zap size={12} style={{ color: c.color }} /><span className="text-[11px] font-bold text-gray-700">Gifting XP</span></div>
      ...
      <div className="mb-3">
        <div className="flex items-center justify-between mb-1.5">
          <div className="flex items-center gap-1"><Coins size={12} className="text-amber-500" /><span className="text-[11px] font-bold text-gray-700">Coins Sent Progress</span></div>
          <span className="text-[11px] font-bold text-amber-500">{currentTier.coins}</span>
        </div>
        <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden">
          <div className="h-full rounded-full" style={{ width: "68%", background: "linear-gradient(to right, #FFC83D, #F59E0B)", boxShadow: "0 0 10px rgba(255,200,61,0.5)" }} />
        </div>
      </div>

      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="rounded-xl p-2.5" style={{ background: `${c.color}08`, border: `1px solid ${c.color}15` }}>
          <div className="flex items-center gap-1 mb-0.5"><TrendingUp size={10} style={{ color: c.color }} /><span className="text-[9px] text-gray-400">Monthly Target</span></div>
          <p className="text-xs font-bold text-gray-700">{u.monthlyTarget}</p>
        </div>
        <div className="rounded-xl p-2.5" style={{ background: "#FFC83D08", border: "1px solid #FFC83D15" }}>
          <div className="flex items-center gap-1 mb-0.5"><Crown size={10} className="text-amber-500" /><span className="text-[9px] text-gray-400">Current Rank</span></div>
          <p className="text-xs font-bold text-gray-700">{u.ranking}</p>
        </div>
      </div>
      ...
      <div className="rounded-2xl p-3 flex items-center gap-3 mb-3" style={{ background: `${c.color}08`, border: `1px solid ${c.color}15` }}>
        ...
        <div className="flex-1">
          <p className="text-[9px] text-gray-400">Next Tier</p>
          <p className="text-xs font-bold text-gray-700">{nextTier?.name || "Supreme Gifter Emperor"}</p>
          <p className="text-[9px] text-gray-400">{nextTier?.levels || "LV200"} • {nextTier?.coins || "50B"} coins</p>
        </div>
        <Crown size={16} style={{ color: "#FFC83D" }} />
      </div>

      <div className="grid grid-cols-3 gap-2">
        {[{ label: "Benefits", icon: Eye }, { label: "Rewards", icon: Gift }, { label: "Details", icon: Crown }].map((a, i) => (
