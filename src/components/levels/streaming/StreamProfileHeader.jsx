      ...
      <div className="relative">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] font-mono text-gray-400 mb-1">{u.streamerId}</p>
            <p className="text-xs font-semibold mb-1" style={{ color: c.color }}>{u.tierName}</p>
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-md bg-blue-50 text-blue-600 border border-blue-100">📡 {u.badge}</span>
        ...
        <div className="grid grid-cols-2 gap-2 mb-3">
          <div className="rounded-xl p-2 text-center" style={{ background: `${c.color}10`, border: `1px solid ${c.color}20` }}>
            <p className="text-sm font-bold" style={{ color: c.color }}>{u.popularityRank}</p>
            <p className="text-[9px] text-gray-400">Popularity Rank</p>
          </div>
          <div className="rounded-xl p-2 text-center" style={{ background: "#FFC83D10", border: "1px solid #FFC83D20" }}>
            <p className="text-sm font-bold text-amber-500">{u.totalStreamCoins}</p>
            <p className="text-[9px] text-gray-400">Stream Coins</p>
          </div>
        </div>
        <div className="grid grid-cols-3 gap-2">
          {[
            { label: "Profile", icon: Eye },
            { label: "Share", icon: Share2 },
            { label: "History", icon: Radio },
          ].map((a, i) => (
