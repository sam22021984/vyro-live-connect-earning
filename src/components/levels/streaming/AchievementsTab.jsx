...
import { streamAchievements, streamMilestones, rarityColors } from "@/components/levels/streaming/streamingData";
import { Lock, Trophy, Check, Crown } from "lucide-react";

export default function AchievementsTab() {
  const unlocked = streamAchievements.filter(a => a.unlocked).length;
  return (
    <div className="space-y-5">
      {/* Streaming Achievements */}
      <div>
        <h3 className="text-sm font-bold text-gray-800 mb-1 px-1">Streaming Achievements</h3>
        <p className="text-[10px] text-gray-400 mb-3 px-1">Broadcaster trophy showcase</p>
        <div className="grid grid-cols-4 gap-2 mb-4">
          {[{ l: "Total", v: streamAchievements.length, i: "🏆", c: "#FFC83D" }, { l: "Unlocked", v: unlocked, i: "✅", c: "#22C55E" }, { l: "Locked", v: streamAchievements.length - unlocked, i: "🔒", c: "#94A3B8" }, { l: "Rare+", v: streamAchievements.filter(a => ["Legendary","Mythic"].includes(a.rarity)).length, i: "⭐", c: "#A855F7" }].map((s, i) => (
      ...
      <div>
        <h3 className="text-sm font-bold text-gray-800 mb-1 px-1">Streaming Milestones</h3>
            ...
            return (
              <div key={i} className="relative mb-3">
                ...
                <div className="rounded-2xl p-3 flex items-center gap-3" style={{ background: "linear-gradient(135deg, #FFFFFF, #F5F7FA)", border: `1px solid ${m.reached ? "#22C55E" : "#E5E7EB"}30`, opacity: m.reached ? 1 : 0.8 }}>
                  ...
                  <button className="text-[9px] font-bold py-1.5 px-2.5 rounded-lg active:scale-95 transition" style={{ background: m.reached ? "#22C55E10" : "#3B82F610", color: m.reached ? "#22C55E" : "#3B82F6" }}>{m.reached ? "Rewards" : "Preview"}</button>
                </div>
              </div>
            );
          })}
