...
import { hostBenefits, nextHostRewards, hostGalleryCategories } from "@/components/levels/host/hostData";
...
export default function RewardsTab() {
  return (
    <div className="space-y-5">
      {/* Host Benefits */}
      <div>
        <h3 className="text-sm font-bold text-gray-800 mb-1 px-1">Host Benefits</h3>
        <p className="text-[10px] text-gray-400 mb-3 px-1">Current host privileges & features</p>
          ...
          {hostBenefits.map((b, i) => (
            <div key={i} className="rounded-2xl p-3" style={{ background: "linear-gradient(135deg, #FFFFFF, #F5F7FA)", border: "1px solid rgba(255,255,255,0.9)", boxShadow: `0 4px 12px ${b.color}15` }}>
              ...
              <div className="grid grid-cols-2 gap-1">
                <button className="text-[8px] font-bold py-1 rounded-lg active:scale-95 transition" style={{ background: b.color, color: "#fff" }}>View</button>
      ...
      <div>
        ...
        <p className="text-[10px] text-gray-400 mb-3 px-1">Upcoming host rewards</p>
        <div className="grid grid-cols-2 gap-2.5">
          {nextHostRewards.map((r, i) => (
      ...
      {/* Host Reward Gallery */}
      <div>
        <h3 className="text-sm font-bold text-gray-800 mb-1 px-1">Host Reward Gallery</h3>
        <p className="text-[10px] text-gray-400 mb-3 px-1">3D host collection gallery</p>
        <div className="grid grid-cols-2 gap-2.5">
          {hostGalleryCategories.map((c, i) => (
