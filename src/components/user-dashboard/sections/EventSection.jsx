...
import { EVENT_DATA } from "../userDashboardData";

export default function EventSection() {
  return (
    <div className="space-y-4">
      <GlassCard>
        <SectionHeader title="Active Events" icon="🎉" />
        <div className="space-y-2">
          {EVENT_DATA.active.map((e, i) => (
      ...
      <GlassCard>
        <SectionHeader title="Completed Events" icon="✅" />
        <div className="space-y-2">
          {EVENT_DATA.completed.map((e, i) => (
            <div key={i} className="flex items-center gap-3 py-2 border-b" style={{ borderColor: "rgba(255,255,255,0.05)" }}>
              <div className="w-9 h-9 rounded-xl flex items-center justify-center text-sm opacity-60" style={{ background: `${e.color}15` }}>{e.icon}</div>
              <div className="flex-1 min-w-0">
                <p className="text-[11px] font-bold text-white">{e.title}</p>
                <p className="text-[9px]" style={{ color: "#10B981" }}>{e.reward}</p>
              </div>
            </div>
          ))}
        </div>
