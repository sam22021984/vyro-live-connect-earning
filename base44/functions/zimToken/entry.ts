...
// ZEGOCLOUD ZIM Token v3 — server-side token for ZEGO Instant Messaging (ZIM) SDK.
// Same V3 token format as Express SDK, but scoped for chat/messaging (no room_id required).
...
Deno.serve(async (req) => {
  try {
    ...
    // Resolve display name from profile (best-effort)
    ...
    const expire = 86400; // 24 hours for messaging sessions
    ...
    console.error('zimToken error:', error?.message || error, error?.stack || '');
